...
The paper contributes to three literatures. First, it adds to the retirement--health literature by showing that the mental-health consequences of retirement depend on local crisis exposure. Second, it contributes to the literature on recessions and health by focusing on older individuals transitioning out of work, a group for whom the value of retirement may change sharply during downturns. Third, it provides evidence from England, where private pension exposure and regional labour-market heterogeneity make the Great Recession a particularly informative setting for studying retirement under macroeconomic stress.


\section{Institutional Background and Motivation} \label{sec:background}
...
To the best of our knowledge, the study by \cite{belloni2016effect} is the only one attempting to estimate the impact of retirement on health while accounting for geographical differences in crisis intensity across Europe. They find that retiring immediately after an economic shock improves mental health, although only among blue-collar men in the most severely affected regions. Our paper differs in three ways. We focus on England, exploit within-person panel variation over a longer horizon, and distinguish between depressive symptoms and new psychiatric diagnosis. This allows us to assess whether any short-run mental-health improvement survives over time and whether it appears consistently across conceptually distinct mental-health outcomes.
...
Because retirement timing may still be correlated with unobserved time-varying determinants of mental health, we instrument retirement with statutory pension-age eligibility. The identifying assumption in the baseline IV specification is that, conditional on the included controls and fixed effects, crossing the statutory pension-age threshold affects mental health only through retirement. Figure~\ref{fig1} shows the sharp increase in retirement around pension eligibility, including the legislated change in women's eligibility age.
...
\tiny{{\singlespacing \emph{Notes:} Appendix Table B2 reports weak-instrument diagnostics for the interacted IV specification in Equation (2), where retirement and retirement $\times$ high unemployment are instrumented with pension-age eligibility and its interaction with high unemployment.}}
\end{table}
